...
public class weapon : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (del > 0) { del -= 1; }
        if (del2==3) { player.GetComponent<Player>().dir = player.GetComponent<Player>().dir * -1; }
        if (del2 > 0) { del2 -= 1; }
        //if (del2==1) { player.GetComponent<Player>().dir = player.GetComponent<Player>().dir * -1; }
        if (player.GetComponent<Player>().cleave && player.GetComponent<Player>().role == 2) { GetComponent<SpriteRenderer>().color = gold; }
        else if (!player.GetComponent<Player>().cleave && player.GetComponent<Player>().role == 2) { GetComponent<SpriteRenderer>().color = white; }
    }
    private void OnCollisionEnter2D(Collision2D col)
    {
        ...
        if (col.gameObject.tag == "Respawn"&&!col.gameObject.GetComponent<xq>().player.GetComponent<Player>().choose)
        {
            ...
            if (player.GetComponent<Player>().role == 1) { HitB.GetComponent<xq>().HP += 3;
            ...
            if (player.GetComponent<Player>().role==3) { player.GetComponent<Player>().cd -= 150; }
            if (player.GetComponent<Player>().role==5) { player.GetComponent<Player>().comDel = 200; }
