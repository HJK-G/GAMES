...
public class Nimble : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        ...
        if (active > 0)
        {
            ...
            if (lungeCd < 11 && lungeCd > 9) { rb.velocity = transform.right * lungePwr; }
            if (lungeCd < 31 && lungeCd > 29) { rb.velocity = transform.right * lungePwr; }
            ...
            if (lungeCd < 70 && lungeCd > 1) { leapDel -= 1; if (leapDel < 1) { GameObject leaping = Instantiate(Leaping, transform.position, transform.rotation); Destroy(leaping, 1.0f); leapDel = 3; } }
    ...
    {
        if (col.gameObject.tag == "Player") { if (lungeCd < 70 && lungeCd > 1) { Ninja.dmg = 50; } }
    }
